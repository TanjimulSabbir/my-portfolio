          ...
          return (
            <>
              <div key={id} className={`relative mt-16 ${index === 0 ? "lg:mt-24" : "lg:mt-32"} lg:w-2/3 mx-auto`}>
                ...
                <a href={Links.live}>
                  <div>
                    <img className="relative w-full max-h-[400px] md:max-w-[400px] md:h-[250px] rounded-lg" src={image} alt="" srcSet="" />
                  </div>
                </a>

                {/* Text Part */}
                <div className="md:absolute md:top-20 pointer-events-none">
                  <h2 className="my-8 md:mt-0 text-custom-highLightTextColor md:text-end">A Quick Glance</h2>
                  <div>
                    <p className="mb-4 bg-[#0d2446] text-xs sm:text-sm p-4 py-5 lg:px-8 rounded text-custom-TextColor shadow-2xl z-20 rubik md:w-[70%] ml-auto pointer-events-auto">{Description}</p>

                    <p className="mb-6 rubik text-sm sm:text-base space-x-1 cursor-pointer text-custom-highLightTextColor text-end md:w-[50%] ml-auto pointer-events-auto">{Technologies.map(technology => <small key={technology}> {technology}</small>)}</p>
